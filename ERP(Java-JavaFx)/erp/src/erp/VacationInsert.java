...
public class VacationInsert { 
	...
	public void tableInsert(int employeeID, String name, int totVac, int useVac, int remainVac){ 
		Vacation vac = new Vacation(employeeID, name, totVac, useVac, remainVac); //외부에서 매개변수로 데이터를 받아 Qurey문을 만들어 줍니다. 
	...
	public void tableSelect() {
		String query = "SELECT * FROM Vacation"; // table에 모든 레코드를 출력하는 query문입니다. 
		Vacation vac = new Vacation(); // TableName 객체를 만들어줍니다. 
		try { // 데이터베이스에 접속합니다.
			Class.forName(JDBC_DRIVER); 
			conn = DriverManager.getConnection(DB_URL,USERNAME,PASSWORD); 
			stmt = conn.createStatement(); //데이터베이스에 query문을 넘길 Statement를 만들어줍니다. 
