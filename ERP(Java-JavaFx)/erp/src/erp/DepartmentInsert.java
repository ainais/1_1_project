...
public class DepartmentInsert { 
	...
	public void tableSelect() {
		String query = "SELECT * FROM Department"; // table에 모든 레코드를 출력하는 query문입니다. 
		...
		try { // 데이터베이스에 접속합니다.
			...
			while (rs.next()){ 
				...
				dep.setChiefDepart(rs.getString("ChiefDepart")); 
				System.out.println("EmployeeID : " + dep.getEmployeeID() + ", Name : " + dep.getName()+ ", Department : "+dep.getDepartment()+ ", EnteringDate : "+dep.ChiefDepart); 
				} 
			stmt.close(); 
			conn.close(); } 
		catch (ClassNotFoundException e) { 
